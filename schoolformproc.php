...
if (isset($_POST['submit'])){
  ...
  if (empty($_POST["phonenumber"])) {
  	...
  	//filter integer
  		# code...
  	if (filter_var($phonenumber, FILTER_VALIDATE_INT) ===0 || !filter_var($integer, FILTER_VALIDATE_INT) === false )
  	 {
  		$phonenumberErr="";
		}
		else
			{
				$phonenumberErr="integer is not valid";
			}
	}
...
if (empty($_POST["studentfirstname"])) {
  ...
  if (empty($_POST["admissionnumber"])) {
    # code...
    $admissionnumberErr="integer is required";
  }else{
    ...
    if (filter_var($admissionnumber, FILTER_VALIDATE_INT) ===0 || !filter_var($admissionnumber, FILTER_VALIDATE_INT) === false )
     {
      ...
      {
        $admissionnumberErr="integer is not valid";
      }
